...
const SanPhamController = {
 ...
 insert: async (req, res) => {
  ...
  try {
    const newSanPham = await SanPhamService.insert(sanPham);
    res.status(201).json({ message: "Thêm sản phẩm thành công.", data: newSanPham });
  } catch (err) {
    ...
    // if (!req.file) {
    //     return res.status(400).json({ message: "Chưa upload được file ảnh." });
    // }
    ...
    //     path: `/uploads/sanpham/${req.file.filename}`,
    //     });
    // } catch (err) {
    //     res.status(500).json({ message: "Lỗi khi upload ảnh đại diện.", error: err.message });
    // }
    // },
