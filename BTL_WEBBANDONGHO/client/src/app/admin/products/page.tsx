...
export default function ProductManagement() {
	...
	const fetchProducts = async () => {
		try {
			...
			setProducts(response.data.sanPhams);
	...
	const handleDeleteProduct = async () => {
		if (selectedProduct) {
			try {
				...
				await axios.delete(`${API_URL}/${selectedProduct.SanPhamID}`);
				toast.current?.show({
					severity: "success",
					summary: "Success",
	...
	return (
		<div>
			...
			<div className="card">
				<Toolbar
					className="mb-4"
					left={<Button label="New" icon="pi pi-plus" severity="success" onClick={() => router.push('/admin/create')} />}
				/>
				<DataTable
					value={products}
					paginator
					...
					<Column
						...
						body={(rowData) => (
							<img
								src={`http://localhost:4000/uploads/sanpham/${rowData.HinhAnh}`}
